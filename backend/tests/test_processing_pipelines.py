...
def test_diff_and_summary_pipelines(session_factory, storage_stub):
    ...
    diff_result = diff_pipeline.run(job_id, old_version_id, new_version_id)

    with session_factory() as session:
        diff_record = session.get(DiffResult, diff_result["diff_result_id"])
    ...
    with session_factory() as session:
        ...
        assert "Summary of detected changes" in latest.summary_text
...
class FakeOrchestrator:
    ...
    def on_diff_complete(self, job_id, diff_result_id):
        self.events.append(("diff", job_id, diff_result_id))
...
def test_diff_and_summary_workers_update_stages(session_factory):
    ...
    diff_worker = DiffWorker(
        pipeline=FakePipeline({"diff_result_id": "diff-1", "result_ref": "diff/ref"}),
    ...
    summary_worker.process_message({"job_id": job_id, "diff_result_id": "diff-1"})
    ...
    with session_factory() as session:
        ...
        assert diff_stage.result_ref == "diff/ref"
        assert summary_stage.status == "completed"
        assert summary_stage.result_ref == "summary-1"

    assert orchestrator.events[-2:] == [("diff", job_id, "diff-1"), ("summary", job_id, None)]

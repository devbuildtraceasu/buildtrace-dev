...
export default function UploadPage() {
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .catch(() => {
        // Ignore failures, default project will be used
      })
    return () => {
      mounted = false
    }
  ...
  const pollJobStatus = async (jobId: string) => {
    ...
    const poll = async () => {
      try {
        ...
        setProcessingSteps(prev => prev.map(step => {
          const stage = stages.stages?.find((s: any) => {
            if (step.id === 'ocr' && s.stage === 'ocr') return true
            if (step.id === 'diff' && s.stage === 'diff') return true
            if (step.id === 'summary' && s.stage === 'summary') return true
            return false
          })
          ...
          if (stage) {
            return {
              ...
              status: stage.status === 'completed' ? 'completed' :
                      ...
                      stage.status === 'in_progress' ? 'active' : step.status
            }
          }
          return step
        }))

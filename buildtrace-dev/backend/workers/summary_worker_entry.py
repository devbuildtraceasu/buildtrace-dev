#!/usr/bin/env python3
"""Entry point for Summary worker service running in Cloud Run."""
import logging
import sys
import os
import threading
...
class HealthHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b'Summary Worker OK')
...
def main():
    ...
    logger.info(f"Starting Summary worker")
    logger.info(f"Project: {config.GCP_PROJECT_ID}")
    logger.info(f"Subscription: {config.PUBSUB_SUMMARY_SUBSCRIPTION}")
    
    try:
        subscriber = PubSubSubscriber(
            project_id=config.GCP_PROJECT_ID,
            subscription_name=config.PUBSUB_SUMMARY_SUBSCRIPTION
        )
        worker = SummaryWorker()
        
        logger.info("Summary worker ready, listening for messages...")
        subscriber.start(worker.process_message)
    except KeyboardInterrupt:
        logger.info("Shutting down Summary worker...")
    except Exception as e:
        logger.exception(f"Fatal error in Summary worker: {e}")
        sys.exit(1)

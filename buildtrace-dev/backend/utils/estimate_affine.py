"""
Constrained Affine Transformation Estimation
Estimates partial 2D affine transformation with constraints on scale and rotation
"""
...
def _run_constrained_optimizer(
    ...
    rotation_deg_min: Optional[float] = None,
    rotation_deg_max: Optional[float] = None,
) -> Optional[np.ndarray]:
...
def estimate_affine_partial_2d_constrained(
    ...
    Args:
        from_points: Source points, shape (N, 2).
        to_points: Destination points, shape (N, 2).
        ransac_reproj_threshold: RANSAC reprojection threshold.
        max_iters: The maximum number of RANSAC iterations.
        confidence: The confidence level for dynamically adapting iterations.
        scale_min: The minimum allowed scale.
        scale_max: The maximum allowed scale.
        rotation_deg_min: The minimum allowed rotation in degrees.
        rotation_deg_max: The maximum allowed rotation in degrees.

    Returns:
        Tuple of (transformation_matrix, inlier_mask):

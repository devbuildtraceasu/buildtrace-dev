...
from gcp.database.models import Session, Drawing, Comparison, AnalysisResult, User, Project, project_users
...
from oauth_config import create_oauth_blueprint
from gcp.database.models import OAuth
...
# Configure proxy handling for Cloud Run
...
# Configure OAuth for HTTPS in production
if config.IS_PRODUCTION:
    # Force HTTPS for OAuth in production
    os.environ['OAUTHLIB_INSECURE_TRANSPORT'] = '0'
    os.environ['OAUTHLIB_RELAX_TOKEN_SCOPE'] = '1'  # Allow scope variations
...
google_bp = make_google_blueprint(
    ...
    redirect_to='oauth_authorized_handler'  # Add explicit redirect handler
)
app.register_blueprint(google_bp, url_prefix="/auth")

# Add error handler for OAuth-specific errors
...
def handle_oauth_state_error(error):
    """Handle OAuth state mismatch errors"""
    logger.error(f"OAuth state mismatch: {error}")
    flash('Authentication session expired. Please try logging in again.', 'error')
    return redirect(url_for('login'))

@app.errorhandler(OAuth2Error)
def handle_oauth2_error(error):
    """Handle general OAuth2 errors"""
...
def load_user(user_id):
    try:
        with get_db_session() as db:
            ...
            if user:
                # Expunge the user from the session to make it detached
                # This allows us to access basic attributes without session issues
                db.expunge(user)
            return user
    except Exception as e:
        logger.error(f"Error loading user {user_id}: {e}")
        # Return None if database is not available - this will log the user out
        return None
...
def login():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash('Unable to process login. Please try again later.', 'error')
            return render_template('auth/login.html')
    ...
    if request.method == 'POST':
        ...
        try:
            with get_db_session() as db:
                ...
                # Add user to project as owner (with denormalized user info)
                db.execute(project_users.insert().values(
                    project_id=default_project.id,
                    user_id=new_user.id,
                    role='owner',
                    joined_at=datetime.utcnow(),
                    invited_by=new_user.id,
                    user_name=new_user.name,
                    email_add=new_user.email,
                    company_name=new_user.company
                ))
                db.commit()

                login_user(new_user, remember=True)
                flash('Account created successfully! Welcome to BuildTrace.', 'success')
                return redirect(url_for('dashboard'))
        except Exception as e:
...
@app.route('/auth/me')
def check_auth():
    """Check current authentication status - for API/frontend"""
    if current_user.is_authenticated:
        return jsonify({
            'success': True,
            'user': {
...
def google_login():
    ...
    return redirect(url_for('oauth_authorized_handler'))

@app.route('/oauth/authorized')
def oauth_authorized_handler():
    """Handle OAuth callback from Google - called after /auth/google/authorized"""
    if not google.authorized:
        flash('Access denied: Unable to authenticate with Google.', 'error')
        return redirect(url_for('login'))

    try:
        # Get user info from Google
        google_info = google.get('/oauth2/v1/userinfo')

        if not google_info.ok:
            flash('Failed to fetch user information from Google.', 'error')
            return redirect(url_for('login'))

        google_data = google_info.json()
        google_user_id = str(google_data['id'])
        ...
        with get_db_session() as db:
            ...
            if user:
                ...
                oauth_record = db.query(OAuth).filter_by(
                    provider='google',
                ...
                # Check if request is from Next.js frontend (has Origin header or Referer)
                origin = request.headers.get('Origin', '')
                referer = request.headers.get('Referer', '')
                is_api_request = 'localhost:3000' in origin or 'localhost:3000' in referer
                
                if is_api_request:
                    # Redirect to Next.js callback page (for local development with Next.js)
                    frontend_url = os.getenv('FRONTEND_URL', 'http://localhost:3000')
                    return redirect(f'{frontend_url}/auth/callback')
                else:
                    # Traditional Flask flow (production uses Flask templates)
                    # Redirect to upload page for immediate functionality
                    next_page = request.args.get('next')
                ...
                # Create default project for the user
                default_project = Project(
                    name=f"{user_name or user_email.split('@')[0]}'s Project",
                    description="Default project created via Google OAuth",
                    status='active',
                    is_default=True
                )
                db.add(default_project)
                ...
                flash(f'Welcome to BuildTrace, {user_name or user_email}!', 'success')
                
                # Check if request is from Next.js frontend
                origin = request.headers.get('Origin', '')
                referer = request.headers.get('Referer', '')
                is_api_request = 'localhost:3000' in origin or 'localhost:3000' in referer
...
def dashboard():
    ...
    # Get user ID outside the database session to avoid DetachedInstanceError
    user_id = current_user.id

    try:
        with get_db_session() as db:
            # Get user's projects (both owned directly and through project_users)
            # Direct ownership (older projects)
            owned_projects = db.query(Project).filter_by(user_id=user_id).all()
            logger.info(f"Dashboard: Found {len(owned_projects)} directly owned projects for user {user_id}")

            # Projects through project_users table (newer projects)
            try:
                shared_projects = db.query(Project).join(project_users).filter(
                    project_users.c.user_id == user_id
                ).all()
                logger.info(f"Dashboard: Found {len(shared_projects)} shared projects for user {user_id}")
            except Exception as e:
                logger.warning(f"Dashboard: Error querying shared projects: {e}")
                shared_projects = []
            ...
            logger.info(f"Dashboard: Total unique projects for user {user_id}: {len(user_projects)}")

            # Get recent sessions for the user
            recent_sessions = db.query(Session).filter_by(user_id=user_id).order_by(
            ...
            for s in recent_sessions:
                sessions_data.append({
                    'id': s.id,
                    'session_type': s.session_type or 'comparison',
        ...
        return render_template('dashboard.html', projects=projects_data, recent_sessions=sessions_data)

    except Exception as e:
        logger.error(f"Database error in dashboard: {e}")
        # If database fails, show empty dashboard with error message
        flash('Unable to load projects and sessions. Database connection error.', 'error')
        return render_template('dashboard.html', projects=[], recent_sessions=[])

@app.route('/project/<project_id>')
@login_required
def project_view(project_id):
    """View a specific project and its sessions"""
    # Get user ID outside the database session to avoid DetachedInstanceError
    user_id = current_user.id

    try:
        with get_db_session() as db:
            # Check if user has access to this project (both direct ownership and project_users)
            ...
            project_sessions = db.query(Session).filter_by(
                project_id=project_id,
                user_id=user_id
            ).order_by(Session.created_at.desc()).all()

            # Convert project to dictionary to avoid session dependency
            project_data = {
                'id': project.id,
                'name': project.name,
        ...
        return render_template('project.html', project=project_data, sessions=sessions_data)

    except Exception as e:
        logger.error(f"Database error in project_view: {e}")
        flash('Unable to load project. Database connection error.', 'error')
        return redirect(url_for('dashboard'))

@app.route('/upload')
@login_required
def upload_page():
    """Upload page with project selection"""
    # Get user ID outside the database session to avoid DetachedInstanceError
    user_id = current_user.id
    ...
    try:
        with get_db_session() as db:
            ...
            # Convert projects to dictionaries to avoid session dependency
            projects_data = [
                {
                    'id': p.id,
                    'name': p.name,
            ...
            if selected_project_id:
                selected_project = db.query(Project).join(project_users).filter(
                ...
                if selected_project:
                    selected_project_data = {
                        ...
                        'location': selected_project.location
                    }

        return render_template('upload.html', projects=projects_data, selected_project=selected_project_data, db_error=False)

    except Exception as e:
        logger.error(f"Database error in upload_page: {e}")
        # If database fails, still show upload page but without project selection
        flash('Unable to load projects due to database connection error.', 'error')
        return render_template('upload.html', projects=[], selected_project=None, db_error=True)
        ...
        try:
            with get_db_session() as db:
                ...
                db.execute(project_users.insert().values(
                    ...
                    email_add=user.email if user else None,
                ...
                # Check if user came from upload page and redirect appropriately
                next_url = request.args.get('next')
                if next_url == 'upload':
                    return redirect(url_for('upload_page', project_id=new_project.id))
                else:
                    return redirect(url_for('project_view', project_id=new_project.id))

        except Exception as e:
            logger.error(f"Database error in create_project: {e}")
            flash('Unable to create project. Database connection error.', 'error')
            return render_template('create_project.html')
...
@app.route('/')
def index():
    """Main landing page - redirect to upload or login"""
    if current_user.is_authenticated:
        return redirect(url_for('upload_page'))
    return redirect(url_for('login'))
...
def get_upload_urls():
    ...
    try:
        ...
        with get_db_session() as db:
            project = db.query(Project).join(project_users).filter(
                Project.id == project_id,
...
def upload_direct_to_storage(storage_path):
    """Handle direct file upload to Cloud Storage for large files - DEPRECATED"""
    # This endpoint is deprecated as it still routes through Cloud Run memory
    # Large files should use /api/upload-stream instead
    return jsonify({'error': 'This endpoint is deprecated. Use chunked upload for large files.'}), 400

@app.route('/api/upload-stream/<path:storage_path>', methods=['PUT'])
def upload_stream_to_storage(storage_path):
    """Stream file upload directly to Cloud Storage without loading into memory"""
    try:
        # Stream the file directly to GCS without loading it all into memory
        content_type = request.headers.get('Content-Type', 'application/pdf')
        content_length = request.headers.get('Content-Length', type=int)

        logger.info(f"Streaming upload to: {storage_path}, size: {content_length} bytes, type: {content_type}")

        # Check if file is too large for direct streaming (>30MB to be safe)
        if content_length and content_length > 30 * 1024 * 1024:
            logger.warning(f"File too large for streaming ({content_length} bytes), client should use chunked upload")
            return jsonify({
        ...
        try:
            ...
            logger.info(f"File streamed successfully to: {storage_path}")
            ...
            logger.error(f"Failed to stream file to storage: {upload_error}")
...
@app.route('/api/initialize-chunked-upload', methods=['POST'])
@login_required
def initialize_chunked_upload():
    """Initialize a chunked upload session for very large files"""
    try:
        data = request.get_json()
        if not data or 'files' not in data:
            return jsonify({'error': 'Missing files data'}), 400
        ...
        if config.USE_DATABASE:
            ...
            with get_db_session() as db:
                session = Session(
                    ...
                    project_id=project_id,
                    status='uploading',
                    session_type='chunked_comparison',
                    created_at=datetime.now(timezone.utc)
                )
                db.add(session)
                db.commit()
        ...
        upload_sessions = {}
        chunk_size = 25 * 1024 * 1024  # 25MB chunks (safe under Cloud Run's 32MB limit)

        for file_info in files:
            if 'name' not in file_info or 'type' not in file_info or 'size' not in file_info:
                return jsonify({'error': 'File name, type, and size required'}), 400
...
def upload_chunk():
    ...
    try:
        ...
        if config.USE_DATABASE:
            # ChunkedUpload models not available - using simple processing
            with get_db_session() as db:
                ...
                existing_chunk = db.query(ChunkedUploadChunk).filter(
                    ChunkedUploadChunk.upload_id == upload_id,
                    ChunkedUploadChunk.chunk_index == chunk_index
                ).first()

                if existing_chunk:
                    logger.info(f"Chunk {chunk_index} already uploaded for upload {upload_id}")
                    return jsonify({'success': True, 'message': 'Chunk already uploaded'})

                # Store chunk in GCS
                chunk_storage_path = f"chunks/{upload_id}/chunk_{chunk_index:04d}"
                chunk_content = chunk_file.read()

                try:
                    import io
                    chunk_stream = io.BytesIO(chunk_content)
                    ...
                    chunk_record = ChunkedUploadChunk(
                        upload_id=upload_id,
                        chunk_index=chunk_index,
                        chunk_size=len(chunk_content),
                        storage_path=chunk_storage_path
                    )
                    db.add(chunk_record)
                    ...
                    logger.info(f"Uploaded chunk {chunk_index} for upload {upload_id} ({upload.chunks_uploaded}/{upload.total_chunks})")
                    ...
                    logger.error(f"Failed to store chunk {chunk_index}: {storage_error}")
                    return jsonify({'error': f'Failed to store chunk: {str(storage_error)}'}), 500
...
@app.route('/api/finalize-chunked-upload', methods=['POST'])
def finalize_chunked_upload():
    """Assemble chunks into final files and start processing"""
    try:
        data = request.get_json()
        if not data or 'session_id' not in data or 'upload_sessions' not in data:
            return jsonify({'error': 'Missing session_id or upload_sessions'}), 400

        session_id = data['session_id']
        upload_sessions = data['upload_sessions']

        if config.USE_DATABASE:
            # ChunkedUpload models not available - using simple processing
            with get_db_session() as db:
                # Get all uploads for this session
                uploads = db.query(ChunkedUpload).filter(
                    ChunkedUpload.session_id == session_id
                ).all()
                ...
                for upload in uploads:
                    if upload.status != 'completed':
                        return jsonify({'error': f'Upload {upload.file_type} not completed'}), 400

                    # Get all chunks for this upload, ordered by index
                    chunks = db.query(ChunkedUploadChunk).filter(
                        ChunkedUploadChunk.upload_id == upload.id
                    ).order_by(ChunkedUploadChunk.chunk_index).all()

                    if len(chunks) != upload.total_chunks:
                        return jsonify({'error': f'Missing chunks for {upload.file_type} file'}), 400

                    # Assemble chunks into final file
                    try:
                        assembled_content = b''
                        for chunk in chunks:
                            chunk_data = storage_service.download_file(chunk.storage_path)
                            assembled_content += chunk_data
                            logger.info(f"Assembled chunk {chunk.chunk_index} for {upload.file_type}")

                        # Upload assembled file to final location
                        import io
                        assembled_stream = io.BytesIO(assembled_content)

                        # Determine content type
                        extension = upload.filename.lower().split('.')[-1] if '.' in upload.filename else ''
                        content_type_map = {
                            'pdf': 'application/pdf',
                            'dwg': 'application/acad',
                            'dxf': 'application/dxf',
                        ...
                        final_storage_path = storage_service.upload_file(
                            assembled_stream,
                            upload.storage_path,
                            content_type=content_type
                        )

                        assembled_files[upload.file_type] = {
                            'storage_path': upload.storage_path,
                            'filename': upload.filename,
                            'size': upload.file_size
                        }

                        logger.info(f"Assembled {upload.file_type} file: {upload.storage_path}")

                        # Clean up chunks (optional - could be done by background job)
                        for chunk in chunks:
                            try:
                                storage_service.delete_file(chunk.storage_path)
                            except Exception as cleanup_error:
                                logger.warning(f"Failed to cleanup chunk {chunk.storage_path}: {cleanup_error}")

                    except Exception as assembly_error:
                        logger.error(f"Failed to assemble {upload.file_type} file: {assembly_error}")
                        return jsonify({'error': f'Failed to assemble {upload.file_type} file'}), 500

                # Start processing the assembled files
                # Create a processing task using the existing chunked processor
                try:
                    if config.USE_CLOUD_TASKS:
                        # Use Cloud Tasks for background processing (if available)
                        try:
                            from gcp.cloud_tasks import task_service
                            if hasattr(task_service, 'create_processing_task'):
                                task_response = task_service.create_processing_task(session_id, assembled_files)
                                logger.info(f"Created Cloud Task for chunked upload session {session_id}")

                                return jsonify({
                                    'success': True,
                                    'session_id': session_id,
                                    'status': 'processing',
                                    'message': 'Files assembled and processing started'
                                })
                            else:
                                logger.warning("Cloud Tasks service not available, falling back to direct processing")
                                raise ImportError("Cloud Tasks not available")
                        except ImportError:
                            logger.warning("Cloud Tasks not available, using direct chunked processor")
                            # Fall through to direct processing

                    # Process using chunked processor directly
                    logger.info(f"Processing chunked upload session {session_id} using chunked processor")

                    # Update session status to processing
                    session = db.query(Session).filter(Session.id == session_id).first()
                    if session:
                        session.status = 'processing'
                        session.baseline_filename = assembled_files['old']['filename']
                        session.revised_filename = assembled_files['new']['filename']
                        db.commit()

                    # Start processing using existing chunked processor
                    old_storage_path = assembled_files['old']['storage_path']
                    new_storage_path = assembled_files['new']['storage_path']

                    # Start processing in a thread to avoid blocking the response
                    import threading
                    def process_in_background():
                        try:
                            # Process the files using the existing chunked processor
                            logger.info(f"Starting background processing for session {session_id}")
                            results = chunked_process_documents(old_storage_path, new_storage_path, session_id)

                            # Store results and update session status
                            _store_results_to_database(session_id, results, old_storage_path, new_storage_path)
                    ...
                    return jsonify({
                        ...
                        'session_id': session_id,
                        'status': 'processing',
                        'message': 'Files assembled and processing started in background'
                    })

                except Exception as task_error:
                    logger.error(f"Failed to start processing: {task_error}")
                    return jsonify({'error': f'Failed to start processing: {str(task_error)}'}), 500
...
def upload_files():
    ...
    try:
        ...
        # Check for project_id or create a default project
        project_id = request.form.get('project_id')

        if project_id:
            # Validate user has access to this project
            with get_db_session() as db:
                # Check both direct ownership and project_users table
                project = db.query(Project).filter(
                    Project.id == project_id,
                    or_(
            ...
            # Create a default project for the user if none provided
            logger.info("No project_id provided, creating default project for user")
            with get_db_session() as db:
                # Check if user has a default project
                default_project = db.query(Project).filter_by(
                    user_id=current_user.id,
                ...
                if not default_project:
                    ...
                    default_project = Project(
                        name='Default Project',
                        description='Automatically created project for drawings',
                        user_id=current_user.id,
                        status='active',
                        created_at=datetime.now(timezone.utc),
                    ...
                    # Also add to project_users table for consistency
                    db.execute(project_users.insert().values(
                        id=str(uuid.uuid4()),
                        project_id=default_project.id,
                        user_id=current_user.id,
                    ...
                    db.commit()
                    logger.info(f"Created default project {default_project.id} for user {current_user.id}")
...
def debug_config():
    ...
    debug_info = {
        ...
        'use_cloud_tasks': config.USE_CLOUD_TASKS,
    ...
    if config.USE_DATABASE:
        try:
            with get_db_session() as db:
                ...
                debug_info['database_connection'] = 'SUCCESS' if result == 1 else 'FAILED'
        except Exception as e:
            debug_info['database_connection'] = f'ERROR: {str(e)}'
    else:
        debug_info['database_connection'] = 'DISABLED'
...
def delete_project(project_id):
    ...
    try:
        with get_db_session() as db:
            # Check if user has access to this project (either owner or has access via project_users)
            project = db.query(Project).filter(
                Project.id == project_id,
                or_(
                    Project.user_id == user_id,
                    exists().where(
                        and_(
                            project_users.c.project_id == project_id,
                            project_users.c.user_id == user_id,
                            project_users.c.role.in_(['owner', 'admin'])  # Only owners/admins can delete
                        )
                    )
                )
            ).first()
            ...
            # Also remove from project_users table
            db.execute(project_users.delete().where(project_users.c.project_id == project_id))
...
def edit_project(project_id):
    ...
    try:
                ...
                or_(
                    ...
                    exists().where(
                        and_(
                            project_users.c.project_id == project_id,
                            project_users.c.user_id == user_id,
                            project_users.c.role.in_(['owner', 'admin'])  # Only owners/admins can edit
                        )
                    )
                )
            ).first()
        ...
        flash('Unable to edit project. Database connection error.', 'error')
        return redirect(url_for('dashboard'))
